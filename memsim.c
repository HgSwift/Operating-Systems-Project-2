...
void LRU(char* filename, int frame_size, char* mode);
...
void LRU(char* filename, int frame_size, char* mode){
   ...
   /*
   Iterates through the file and every line the int count grows
   when a frame is used the same position in a different array
   is assigned the value of count. Meaning that the least recently
   used frame will have the same positon as the lowest value in the
   recent array.   
   */
    srand(time(0));
    Page frames[frame_size];
    ...
    while(fscanf(file,"%x %c",&addr,&rw) != EOF){
        ...
        if(rw == 'R'){
            ...
            for(int i = 0; i < frame_size; i++){
                // if we find the relevant page, continue
                // and set previous page to the index we found it at
                // after this set the corresponding value in recent to count
                // this is to take advantage of principle of locality
                // to avoid iterating through the entire array for multiple
                // reads of the same page
                if(frames[i].address == addr){
            ...
            // if the page was not found search for least recently used page.
            if(!found){
                int comp = count;
                int evict;
                // Iterates through recent to find the position of the lowest value
            ...
            for(int i = 0; i < frame_size; i++){
                ...
                // if we find the relevant page, change its dirty bit
                // and set previous page to the index we found it at
                // then the corresponding value in recent is set to count
                // this is to take advantage of principle of locality
                // to avoid iterating through the entire array for multiple
                // reads of the same page
                if(frames[i].address == addr){
            ...
            if(!found){
                ...
                newPage.address = addr;
                newPage.dirtyBit = 1;
                // if the page to be evicted has been modified
                // increment writes and read
                if(frames[evict].dirtyBit == 1){
                    writes++;
                    reads++;
